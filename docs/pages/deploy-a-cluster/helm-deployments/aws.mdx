...
The `teleport-cluster` chart can deploy either a Kubernetes `LoadBalancer` or Kubernetes `Ingress` to handle
incoming connections to the Teleport Proxy Service.
...
You can use [`cert-manager`](https://cert-manager.io/docs/) to provision and automatically renew TLS credentials
...
This method uses a Kubernetes `Ingress`, which can provision an underlying
[AWS Application Load Balancer (ALB)](https://docs.aws.amazon.com/elasticloadbalancing/latest/application/introduction.html)
to handle incoming traffic if one does not already exist. It also requires the installation and setup of the [AWS Load Balancer controller](https://kubernetes-sigs.github.io/aws-load-balancer-controller).

You should be aware of these potential limitations when using Layer 7 load balancers with Teleport:

- Connecting to Kubernetes clusters or databases at the command line via `tsh` requires the use
  of the `tsh proxy kube` and `tsh proxy db` commands respectively. It is not possible to connect `kubectl`
  directly to the Teleport Kubernetes listener in this mode.
- The use of `tsh` v13 or higher is required.

<Admonition type="warning">
  This method requires that your cluster has a fully functional installation of the AWS Load Balancer
  controller with required IAM permissions. This documentation links to AWS-managed documentation to provide
...
certificate ARN to use, the port it should be served on and other ALB configuration
parameters.
...
Edit your `values.yaml` file to complete the `annotations.ingress` field as follows:
