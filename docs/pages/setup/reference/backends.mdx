...
- `use_fips_endpoint=true` -  set to `true` or `false`. If `true`, [FIPS](https://aws.amazon.com/compliance/fips/) S3 endpoints will be used. If `false`, normal S3 endpoints will be used. If unset, the AWS Environment Variable `AWS_USE_FIPS_ENDPOINT` will determine which endpoint is used. In Teleport 10 or greater, FIPS endpoints will be used if Teleport is run with the `--fips` flag.
