...
- `use_fips_endpoint=true` -  set to `true` or `false`. If `true`, [FIPS](https://aws.amazon.com/compliance/fips/) S3 endpoints will be used. If `false`, normal S3 endpoints will be used. If this parameter is unset, Teleport will use the value of the environment variable `AWS_USE_FIPS_ENDPOINT` to determine which endpoint to use use. FIPS endpoints will also be used if Teleport is run with the `--fips` flag.
...
The optional `GET` parameters shown below control how Teleport interacts with a Dynamo endpoint.
...
- `use_fips_endpoint=true` -  set to `true` or `false`. If `true`, [FIPS](https://aws.amazon.com/compliance/fips/) Dynamo endpoints will be used. If `false`, normal Dynamo endpoints will be used. If unset, the AWS Environment Variable `AWS_USE_FIPS_ENDPOINT` will determine which endpoint is used. FIPS endpoints will also be used if Teleport is run with the `--fips` flag.
...
Starting from version 9.0.4, when running on a filesystem that supports file locks (i.e. a local filesystem,
