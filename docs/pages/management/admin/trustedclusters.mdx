...
When a user tries to connect to any resource inside the leaf cluster using the root's proxy, the
...
- The `tctl` admin tool and `tsh` client tool version >= (=cloud.version=).
  To download these tools, visit the [Downloads](../../deploy-a-cluster/teleport-cloud/downloads.mdx) page.
...
## Step 1/6. Prepare your environment
...
## Step 2/6. Establish trust between clusters
...
## Step 3/6. Manage access to your Trusted Cluster
...
## Step 4/6. Access a Node in your remote cluster
...
## Step 5/6. Updating role mappings

You can update the role mappings by modifying the trusted_cluster.yaml file we created
earlier to contain the new role mapping.

After logging into the leaf cluster:

```code
$ tctl create --force updated_cluster.yaml
```

## Step 6/6. Remove trust between your clusters
