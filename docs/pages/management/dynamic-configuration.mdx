...
configuration without redeploying Teleport instances. 
...
Teleport borrows the concept of a dynamic configuration resource from the REST
programming pattern. A cluster is composed of different objects (i.e.,
resources) and there are three common operations that can be performed on them:
`get` , `create` , and `remove` .
...
- [Get started with the Terraform
  provider](./dynamic-configuration/terraform-provider.mdx).
- [Use Teleport's Terraform provider with
  Spacelift](./dynamic-configuration/spacelift.mdx).
