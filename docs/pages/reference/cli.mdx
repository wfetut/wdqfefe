...
- [teleport](#teleport): Supports the Teleport access plane by starting and configuring various Teleport services.
...
The CLI tool that supports the Teleport access plane is called `teleport`, and allows Teleport services to be managed
...
| `teleport start` | Starts the `teleport` process in the foregound using the current shell session, including any services configured by the [configuration YAML file](config.mdx). |
