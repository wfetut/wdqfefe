...
After authentication Teleport will add the GitHub username
to that user's `internal.logins` trait for their session. The preset `access`
role has this trait variable configured to include the GitHub user as an
authorized SSH login. Here is an example role configuration snippet using the trait variable. 
