...
The commercial edition of Teleport allows users to log in and retrieve their SSH
...
<TileSet>
  <Tile icon="lock" title="Azure AD" href="./azuread.mdx">
    Azure Active Directory (AD) SSO.
  </Tile>
  <Tile icon="lock" title="ADFS" href="./adfs.mdx">
    Active Directory (ADFS) SSO.
  </Tile>
  <Tile icon="lock" title="Google Workspace" href="./google-workspace.mdx">
    Google Workspace SSO.
  </Tile>
  <Tile icon="lock" title="GitLab" href="./gitlab.mdx">
    GitLab SSO.
  </Tile>
  <Tile icon="lock" title="GitHub" href="./github.mdx">
    GitHub SSO.
  </Tile>
  <Tile icon="lock" title="OneLogin" href="./one-login.mdx">
    OneLogin SSO.
  </Tile>
  <Tile icon="lock" title="OpenID Connect" href="./oidc.mdx">
    OpenID Connect (OIDC) SSO.
  </Tile>
  <Tile icon="lock" title="Okta" href="./okta.mdx">
    Okta SSO.
  </Tile>
</TileSet>
...
# Output:
If the browser window does not open automatically, open it by clicking on the link:
...
*authentication connector*. An auth connector is a plugin that controls how
...
metadata:
  name: corporate
spec:
  # Display allows setting the caption of the "login" button
...
- You may use `entity_descriptor_url`, in place of `entity_descriptor`, to fetch the entity descriptor from your IDP. Though, we recommend "pinning" the entity descriptor by including the XML rather than fetching from a URL.
...
- `adfs` (SAML): Required for compatibility with Active Directory (ADFS); refer
  to the full [ADFS guide](./adfs.mdx#create-teleport-roles) for details.
...
same as the first part of their email address - the 'local' part. For example, `dave.smith@acme.com` might log in with the username `dave.smith`. Teleport provides an easy way to
...
- Double-check the hostnames, tokens, and TCP ports in a connector definition.
