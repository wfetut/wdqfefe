...
{/* TODO: Add Helm instructions here*/}
...
The Jira Access Request plugin serves a Jira webhook over HTTP. The Teleport
Access Request plugin uses this webhook to listen for changes in tasks that have
to do with Access Requests and manages those Access Requests in response.
...
You can skip this step if you are running the plugin on the Teleport Proxy
Service host. Later, you will configure the plugin to use the same TLS
credentials that the Proxy Service uses to serve the Teleport Web UI.
...
your domain name with the IP address of the webhook. Here are some examples of
how to create a domain name on AWS and Google Cloud, both for a Linux host and
Kubernetes.
