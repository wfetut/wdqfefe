...
## Step 3/8. Configure HTTPS for the plugin
...
HTTPS on your Jira plugin, we will use the Electronic Frontier Foundation's
Certbot to communicate with Let's Encrypt to fetch a free certificate and
private key.

{/* TODO: Add the DNS include--or modify it for the plugin--since we'll set up a
separate domain name for the plugin */}

{/* TODO: add instructions for using Caddy*/}
...
The `[http]` setting block describes how the plugin's HTTP server works. The
HTTP server is responsible for listening for updates from Jira, and processing
updates, like when someone drags a task from Inbox to Approved column.

You must provide an address the server should listen on, and a certificate to
use. It's possible to run the Jira plugin on the same server as the Teleport
Proxy, so you can use the same TLS certificate.
