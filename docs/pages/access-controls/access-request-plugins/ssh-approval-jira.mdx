...
The Jira plugin performs a two-way synchronization between a dedicated project
board and the Access Requests processed by your Teleport cluster. When you
change the status of an Access Request within Teleport, the plugin updates the
board. And when you update the status of an Access Request on the board, the
plugin updates the Access Request in Teleport.
...
- An environment on AWS or Google Cloud where you will run the Jira plugin. This
  ...
  - A Kubernetes cluster deployed via a cloud provider. The Teleport Jira plugin
    Helm chart uses a cloud-managed load balancer to direct traffic to the
    webhook.
...
access both your Teleport Cloud tenant and the Jira API. The installation
instructions depend on whether you are installing the plugin as a binary or on
Kubernetes via Helm: 
...
Set up your environment to request TLS credentials from Let's Encrypt.
...
functionality. We will use it only to provide a TLS key and certificate to the
Jira plugin.
...
Unset "Connect repositories, documents, and more", then click **Next**.
...
## Step 6/8. Configure the plugin

Earlier, you retrieved credentials that the Jira plugin will use to connect to
...
generated earlier via Caddy. Use the following values, assigning <Var
name="example.com" /> to the domain name you created for the plugin earlier:
...
`publicAddress` is the public address of your webhook. This is the domain name
you created for your webhook. (We will create a DNS record for this domain name
later.)

`tlsFromSecret` is the name of a Kubernetes secret containing TLS credentials
for the webhook. Use `teleport-plugin-jira-webhook`. We will create a
`Certificate` resource later that will populate this secret with TLS
credentials.
...
### Install the plugin's Helm chart
...
At this point, neither Let's Encrypt nor Jira will be able to reach the Jira
webhook. Create a DNS record that associates the webhook's domain name with the
address of the Kubernetes load balancer created by the Helm chart.

See whether the load balancer has a domain name or IP address:

```code
$ kubectl -n teleport get services
NAME                   TYPE           CLUSTER-IP       EXTERNAL-IP                                                             PORT(S)         AGE
kubernetes             ClusterIP      10.100.0.1       <none>                                                                  443/TCP         116m
teleport-plugin-jira   LoadBalancer   10.100.213.245   abc123.us-west-2.elb.amazonaws.com   443:30720/TCP   53s
```
...
To request a certificate for the Teleport Jira plugin, creatge a cert-manager
...
kind: Certificate
metadata:
  name: teleport-plugin-jira-webhook
  namespace: teleport 
spec:
  secretName: teleport-plugin-jira-webhook
  dnsNames:
  - "example.com"
  issuerRef:
    name: teleport-plugin-jira-tls
