...
<Notice type="tip" scope="enterprise">

You can skip this step if you are running the plugin on the same host or
Kubernetes cluster as the Teleport Proxy Service. Later, you can configure the
plugin to use the same TLS credentials that the Proxy Service uses to serve the
Teleport Web UI.
...
your domain name with the Jira webhook. Here are some examples of how to create
a domain name on AWS and Google Cloud, both for a Linux host and Kubernetes.

<Tabs dropdownSelected="Linux Host" dropdownCaption="Environment type">
<TabItem options="Linux Host" label="AWS Route 53">
    ```code
...
<TabItem options="Linux Host" label="GCP Cloud DNS">
    ```code
    $ MYZONE="myzone"
    # The fully qualified domain name for your Teleport Proxy Service.
    # These commands will also create an A record for a wildcard subdomain.
...
Switch your Kubernetes context to the namespace you are using for your Teleport
resources and run the following commands:
    ...
    # These commands will also create an A record for a wildcard subdomain.
    $ MYDNS="tele.example.com"
    $ MYIP=$(kubectl get service teleport-jira-plugin -o jsonpath='{.status.loadBalancer.ingress[0].ip }')
