...
Images with the `*-distroless` suffix within the repository name include only
the `teleport` binary and its runtime dependencies, with no shell or utility
applications. An example is `public.ecr.aws/gravitational/teleport-distroless` for Teleport
Community Edition.
...
recommend their use to keep your Teleport installation up to date.
...
|-|-|-|
|`public.ecr.aws/gravitational/teleport-distroless:(=teleport.major_version=)`|No|[Distroless Debian 11](https://github.com/GoogleContainerTools/distroless)|
|`public.ecr.aws/gravitational/teleport-distroless-debug:(=teleport.major_version=)`|Yes|[Distroless Debian 11](https://github.com/GoogleContainerTools/distroless)|
...
| - | - | - | - |
| `public.ecr.aws/gravitational/teleport-ent-distroless:(=teleport.major_version=)` | No | [Distroless Debian 11](https://github.com/GoogleContainerTools/distroless) |
| `public.ecr.aws/gravitational/teleport-ent-distroless-debug:(=teleport.major_version=)` | Yes | [Distroless Debian 11](https://github.com/GoogleContainerTools/distroless) |

We also provide the following image for nightly FIPS builds of Teleport
Enterprise:

| Image name | Includes troubleshooting tools | Image base |
| - | - | - |
| `public.ecr.aws/gravitational/teleport-ent:(=teleport.major_version=)-fips` | Yes | [Ubuntu 20.04](https://hub.docker.com/\_/ubuntu) |
...
These images point to a static version of Teleport. Use the image's tag to
specify the major, minor, and patch version. For example, you can pull an image
of Teleport Community Edition version `(=teleport.version=)` by using the
following image name: `(=teleport.latest_oss_docker_image=)`.
...
For FIPS builds of Teleport Enterprise with a specific version, you can use the
following container image:

|Image name|Troubleshooting Tools?|Image base|
|-|-|-|
| `(=teleport.latest_ent_docker_image=)-fips` | Yes | [Ubuntu 20.04](https://hub.docker.com/\_/ubuntu) |
...
Our [Helm charts](#helm) take care of the Teleport container's system
requirements for you if deploying Teleport on Kubernetes, so these instructions
are relevant only when running a standalone Teleport container in some other
environment.
...
need to mount this directory to your Teleport container, then assign the
following fields in the container's configuration file to the appropriate paths:
...
Services on a local Docker container.
