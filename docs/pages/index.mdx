...
For most Teleport users, it should be possible to protect your infrastructure
with Teleport by following the instructions in our [Fast Path to
Teleport](fast-path/introduction.mdx) learning track, which shows you how to set
up a full role-based access control system for Kubernetes with single sign-on
via GitHub in under an hour.
