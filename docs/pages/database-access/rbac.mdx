---
title: Database Access Access Controls
...
1. *database administrators* have access to everything.
2. *QA and engineering* teams have full access to staging databases.
3. *engineers* can gain temporary access to the production database in case of emergency.

Each role is defined by specific permissions and limits on access. 

Read [Role-Based Access Control](../access-controls/introduction.mdx) for a general overview about RBAC Enterprise support.
...
It's possible to use wildcards to match any database names/users.
...
contain multiple schemas. To change to another database, a user disconnects from the current one and establishes a new connection. During a PostgreSQL connection attempt, `db_names` field is checked against the name
of the logical database that the user is connecting to.

In MySQL, a logical "database" and a "schema" are synonyms for each other, and the scope of permissions a user has once connected is determined by the permission grants set on the account within the database. As such, `db_names` role field
...
The `{{external.xyz}}` variables are replaced with values from external [SSO](../access-controls/single-sign-on/index.mdx)
providers. For OIDC, they will be expanded with a value of an "xyz" claim; for SAML — with an "xyz" assertion value.
...
The `{{internal.db_users}}` and `{{internal.db_names}}` variables permit sharing allowed database accounts and names with remote clusters. They will be replaced
