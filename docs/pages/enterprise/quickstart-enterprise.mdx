...
- **Proxy servers** route client connection requests to the appropriate node and serve a Web UI that can also be used to log into SSH nodes. Every client-to-node
...
- **Nodes** provide access to resources including SSH, Kubernetes, web applications
  , and databases.  In this quick start guide, we are highlighting the SSH service. This SSH service is similar to the `sshd` daemon you may be familiar with. When a node receives a connection request, the request is authenticated through the cluster's auth server.
...
| tsh | Allows users to authenticate and access resources via their local machine. `tsh`'s ssh functionality is similar in principle to OpenSSH's `ssh`. Users can log in into remote SSH nodes, list and search for nodes in a cluster, securely upload/download files, etc. |
...
Linux distribution such as Debian, Ubuntu, or a RHEL derivative. `node.example.com` must be able to connect to `auth.example.com` `3025` port and `auth.example.com` must be able to connect to the `3022` port on `node.example.com`.
...
listening on  on [TCP/IP ports](../admin-guide.mdx#ports). On `auth.example.com`, it should
...
See [troubleshooting](#troubleshooting) section below for further debugging and troubleshooting options.
...
see its definition by executing `sudo tctl get roles/admin > admin-role.yaml`.
...
Now, let's create a new Teleport user "joe" with "admin" role:

```bash
sudo tctl users add --roles=admin joe

# The signup token has been created and is valid for 1 hour. Share this URL with the user:
...
the 2nd-factor authentication. We recommend [Google Authenticator](https://en.wikipedia.org/wiki/Google_Authenticator)
...
But first, let's see how Joe can log into the Teleport cluster. He can do this
...
in Teleport always must go through an SSH proxy, sometimes called an "SSH bastion".

<Admonition
  type="warning"
  title="Warning"
>
  For this quickstart, we are using the `--insecure` flag which allows us to skip configuring the HTTP/TLS certificate for Teleport proxy. Your browser will throw a warning **Your connection is not private**. Click Advanced, and **Proceed to 0.0.0.0 (unsafe)**
  to preview the Teleport UI.

  Never use `--insecure` in production unless you terminate SSL at a load balancer. This will apply to most cloud providers (AWS, GCP, and Azure). You must configure an HTTP/TLS certificate for the Proxy.
...
Teleport Enterprise should be using a Single Sign-On (SSO) mechanism that uses SAML or OIDC protocols.

Take a look at the [SSH via Single Sign-on](../access-controls/single-sign-on/index.mdx) chapter to learn the basics of
integrating Teleport with SSO providers. We have the following detailed guides for
configuring SSO providers:

- [Okta](../access-controls/single-sign-on/okta.mdx)
- [Active Directory](../access-controls/single-sign-on/adfs.mdx)
- [One Login](../access-controls/single-sign-on/one-login.mdx)
...
This table gives an idea of how our image naming scheme works. We offer images that point to a static version of Teleport Enterprise, as well as images that are
...
You can now [follow this guide from "Adding Users"](#step-33-adding-users) onwards to create your user and log into Teleport Enterprise.
...
- Network issues: port `3025` is closed via iptables and/or cloud provider security group rules.
