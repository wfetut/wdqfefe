...
### Determine if you need to query transitive groups

{/* TODO: 

Very quickly explain the different APIs available here and how they require 
different paid Google Workspace plans.

If you have access to one level of service, you can use x API and query
transitive groups. If not, what do you need to do in Teleport and Google
Workspace?

/*}

### Determine your service account scope

{/* TODO: 

Explain that you can determine the scope based on your service account.
...
{/* TODO: 
  ...
  Explain that v3 connectors grab the JSON credentials from the service account
  URL and attempt to get groups from Google Cloud Identity. If that doesn't work,
  they use the Admin Directory Group Read Only scope (sp?).

/*}

{/* TODO:
  Explain that V2 connectors always use the `AdminDirectoryGroupReadonlyScope`.
/*}

{/* TODO: move the paragraphs below into the structure above */}
...
that is not possible for users that belong to groups external to the
workspace, those users will not be able to log in to Teleport.

We recommend blocking access to external groups at the workspace level
(following [these instructions](https://support.google.com/a/answer/9468710))
when using the transitive group feature.

The [Groups Inspection](https://admin.google.com/ac/groupsinspection) page in
the Google Admin Console has the same limitations as Teleport in this regard,
and can be used as a first step to diagnose login issues pertaining to the
transitive groups feature.

{/* TODO: 

move the content of this Details box to a more appropriate place
