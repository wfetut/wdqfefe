...
func NewWithConfig(ctx context.Context, cfg Config) (*Backend, error) {
	...
	if !cfg.Memory {
		...
		//connectorURL = fmt.Sprintf("file:%v?_busy_timeout=%v&_sync=%v&cache=shared", fullPath, 10*cfg.BusyTimeout, cfg.Sync)
		//connectorURL = fmt.Sprintf("file:%v?_busy_timeout=%v&_sync=%v&mode=rwc", fullPath, 10*cfg.BusyTimeout, cfg.Sync)
		connectorURL = fmt.Sprintf("file:%v?cache=private&mode=rwc&_busy_timeout=10000&_journal_mode=WAL", fullPath)
	} else {
