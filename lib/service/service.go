...
func initAuthExternalAuditLog(ctx context.Context, auditConfig types.ClusterAuditConfig, backend backend.Backend, tracingProvider *tracing.Provider, log logrus.FieldLogger) (events.AuditLogger, error) {
	...
	for _, eventsURI := range auditConfig.AuditEventsURIs() {
		...
		case pgevents.Schema, pgevents.AltSchema:
			...
			cfg.SetFromAuditConfig(auditConfig)
			cfg.SetLogFromParent(log)
...
func (process *TeleportProcess) initAuthService() error {
	...
	if cfg.Auth.NoAudit {
		...
		externalLog, err := initAuthExternalAuditLog(process.ExitContext(), cfg.Auth.AuditConfig, process.backend, process.TracingProvider, process.log)
		if err != nil {
			if !trace.IsNotFound(err) {
				return trace.Wrap(err)
			}
