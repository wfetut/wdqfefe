...
func (cfg *SessionWriterConfig) CheckAndSetDefaults() error {
	...
	if cfg.ClusterName == "" {
		return trace.BadParameter("session writer config: missing parameter ClusterName")
	}
	if cfg.Namespace == "" {
...
// SessionWriter wraps session stream
...
func (a *SessionWriter) RecordEvent(ctx context.Context, event apievents.AuditEvent) error {
	if err := checkEventFields(event); err != nil {
...
func checkEventFields(event apievents.AuditEvent) error {
	...
	if ok && sess.GetSessionID() == "" {
		return trace.BadParameter("SessionID is unset, call SetupEvent before recording events")
	}
	srv, ok := event.(ServerMetadataGetter)
	if ok && (srv.GetServerID() == "" || srv.GetServerNamespace() == "") {
		return trace.BadParameter("ServerMetadata is unset, call SetupEvent before recording events")
	}
...
// SetupEvent will set necessary event fields, and must be called before
// the event is recorded.
func (a *SessionWriter) SetupEvent(event apievents.AuditEvent) error {
	a.mtx.Lock()
	defer a.mtx.Unlock()
