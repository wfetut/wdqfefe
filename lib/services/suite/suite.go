...
func userSlicesEqual(t *testing.T, a []types.User, b []types.User) {
	...
	require.Equal(t, len(a), len(b), comment)
...
func usersEqual(t *testing.T, a types.User, b types.User) {
	comment := check.Commentf("a: %#v b: %#v", a, b)
	require.True(t, services.UsersEquals(a, b), comment)
}
...
func (s *ServicesTestSuite) UsersCRUD(t *testing.T) {
	...
	require.Nil(t, s.WebS.UpsertPasswordHash("user1", []byte("hash")))
	require.Nil(t, s.WebS.UpsertPasswordHash("user2", []byte("hash2")))
	...
	require.Nil(t, s.WebS.UpsertUser(user))
	...
	require.Nil(t, s.WebS.DeleteUser(ctx, "user1"))
...
func (s *ServicesTestSuite) LoginAttempts(t *testing.T) {
	...
	attempts, err = s.WebS.GetUserLoginAttempts(user.GetName())
	require.NoError(t, err)
	fixtures.DeepCompare(t, attempts, []services.LoginAttempt{attempt1, attempt2})
...
func (s *ServicesTestSuite) CertAuthCRUD(t *testing.T) {
	...
	fixtures.DeepCompare(t, cas[0], ca2)

	cas, err = s.CAS.GetCertAuthorities(ctx, types.UserCA, true)
	require.NoError(t, err)
	fixtures.DeepCompare(t, cas[0], ca)
	...
	ca = NewTestCA(types.UserCA, "example.com")
	require.Nil(t, s.CAS.CreateCertAuthority(ca))
	...
	fixtures.DeepCompare(t, &newCA, out)
}
...
func (s *ServicesTestSuite) ServerCRUD(t *testing.T) {
	...
	require.Nil(t, s.PresenceS.UpsertProxy(proxy))

	out, err = s.PresenceS.GetProxies()
	require.NoError(t, err)
	require.Len(t, out, 1)
	proxy.SetResourceID(out[0].GetResourceID())
	fixtures.DeepCompare(t, out, []types.Server{proxy})
	...
	require.Nil(t, s.PresenceS.UpsertAuthServer(auth))

	out, err = s.PresenceS.GetAuthServers()
	require.NoError(t, err)
	require.Len(t, out, 1)
	auth.SetResourceID(out[0].GetResourceID())
	fixtures.DeepCompare(t, out, []types.Server{auth})
	...
	fixtures.DeepCompare(t, out, []types.Server{kube1, kube2})

	require.Nil(t, s.PresenceS.DeleteKubeService(ctx, kube1.GetName()))
	out, err = s.PresenceS.GetKubeServices(ctx)
	require.NoError(t, err)
	require.Len(t, out, 1)
	fixtures.DeepCompare(t, out, []types.Server{kube2})

	require.Nil(t, s.PresenceS.DeleteAllKubeServices(ctx))
	out, err = s.PresenceS.GetKubeServices(ctx)
	require.NoError(t, err)
	require.Len(t, out, 0)
}
...
func (s *ServicesTestSuite) AppServerCRUD(t *testing.T) {
	...
	fixtures.DeepCompare(t, []types.Server{server}, out)
...
func (s *ServicesTestSuite) ReverseTunnelsCRUD(t *testing.T) {
	...
	require.Nil(t, s.PresenceS.UpsertReverseTunnel(tunnel))

	out, err = s.PresenceS.GetReverseTunnels(context.Background())
	require.NoError(t, err)
	require.Len(t, out, 1)
	tunnel.SetResourceID(out[0].GetResourceID())
	fixtures.DeepCompare(t, out, []types.ReverseTunnel{tunnel})
...
func (s *ServicesTestSuite) PasswordHashCRUD(t *testing.T) {
	...
	require.True(t, trace.IsNotFound(err), check.Commentf("%#v", err))
	...
	require.NoError(t, err)
	fixtures.DeepCompare(t, hash, []byte("hello123"))
	...
	hash, err = s.WebS.GetPasswordHash("user1")
	require.NoError(t, err)
	fixtures.DeepCompare(t, hash, []byte("hello321"))
}
...
func (s *ServicesTestSuite) WebSessionCRUD(t *testing.T) {
	...
	require.Equal(t, trace.IsNotFound(err), true, check.Commentf("%#v", err))
	...
	require.NoError(t, err)
	fixtures.DeepCompare(t, out2, ws1)

	require.Nil(t, s.WebS.WebSessions().Delete(ctx, types.DeleteWebSessionRequest{
		User:      req.User,
...
func (s *ServicesTestSuite) TokenCRUD(t *testing.T) {
	...
	token, err := types.NewProvisionToken("token", types.SystemRoles{types.RoleAuth, types.RoleNode}, time.Time{})
	require.NoError(t, err)

	require.Nil(t, s.ProvisioningS.UpsertToken(ctx, token))
	...
	require.Nil(t, s.ProvisioningS.DeleteToken(ctx, "token"))
	...
	// compatible
	fixtures.DeepCompare(t, v1.V2(), v2)
	fixtures.DeepCompare(t, v2.V1(), v1)
	...
	require.NoError(t, err)
	fixtures.DeepCompare(t, out, v2)

	// Test delete all tokens
	token, err = types.NewProvisionToken("token1", types.SystemRoles{types.RoleAuth, types.RoleNode}, time.Time{})
	require.NoError(t, err)
	require.Nil(t, s.ProvisioningS.UpsertToken(ctx, token))

	token, err = types.NewProvisionToken("token2", types.SystemRoles{types.RoleAuth, types.RoleNode}, time.Time{})
	require.NoError(t, err)
	require.Nil(t, s.ProvisioningS.UpsertToken(ctx, token))
...
func (s *ServicesTestSuite) RolesCRUD(t *testing.T) {
	...
	fixtures.DeepCompare(t, rout, &role)
...
func (s *ServicesTestSuite) NamespacesCRUD(t *testing.T) {
	...
	require.NoError(t, err)
	fixtures.DeepCompare(t, nsout, &ns)
...
func (s *ServicesTestSuite) SAMLCRUD(t *testing.T) {
	...
	require.NoError(t, err)
	fixtures.DeepCompare(t, out, connector)

	connectors, err := s.WebS.GetSAMLConnectors(ctx, true)
	require.NoError(t, err)
	fixtures.DeepCompare(t, []types.SAMLConnector{connector}, connectors)
	...
	fixtures.DeepCompare(t, out2, &connectorNoSecrets)

	connectorsNoSecrets, err := s.WebS.GetSAMLConnectors(ctx, false)
	require.NoError(t, err)
	fixtures.DeepCompare(t, []types.SAMLConnector{&connectorNoSecrets}, connectorsNoSecrets)

	err = s.WebS.DeleteSAMLConnector(ctx, connector.GetName())
	require.NoError(t, err)

	err = s.WebS.DeleteSAMLConnector(ctx, connector.GetName())
	require.Equal(t, trace.IsNotFound(err), true, check.Commentf("expected not found, got %T", err))
...
func (s *ServicesTestSuite) TunnelConnectionsCRUD(t *testing.T) {
	...
	fixtures.DeepCompare(t, out[0], conn)
...
func (s *ServicesTestSuite) GithubConnectorCRUD(t *testing.T) {
	...
	require.NoError(t, err)
	fixtures.DeepCompare(t, []types.GithubConnector{connector}, connectors)
	...
	require.NoError(t, err)
	fixtures.DeepCompare(t, []types.GithubConnector{&connectorNoSecrets}, connectorsNoSecrets)

	err = s.WebS.DeleteGithubConnector(ctx, connector.GetName())
	require.NoError(t, err)

	err = s.WebS.DeleteGithubConnector(ctx, connector.GetName())
	require.True(t, trace.IsNotFound(err), check.Commentf("expected not found, got %T", err))
...
func (s *ServicesTestSuite) RemoteClustersCRUD(t *testing.T) {
	...
	fixtures.DeepCompare(t, out[0], rc)
...
func (s *ServicesTestSuite) StaticTokens(t *testing.T) {
	...
	fixtures.DeepCompare(t, staticTokens, out)
...
func (s *ServicesTestSuite) ClusterName(t *testing.T, opts ...Option) {
	...
	fixtures.DeepCompare(t, clusterName, gotName)
...
func (s *ServicesTestSuite) SemaphoreContention(t *testing.T) {
	...
	for i := 0; i < iters; i++ {
		...
		wg.Wait()
		cancel()
		require.Nil(t, s.PresenceS.DeleteSemaphore(ctx, types.SemaphoreFilter{
	...
	testCases := []eventTest{
		{
			...
			crud: func(context.Context) types.Resource {
				...
				require.Nil(t, s.CAS.DeleteCertAuthority(*ca.ID()))
				return out
			},
		},
	}
	...
	testCases = []eventTest{
		{
			...
			crud: func(context.Context) types.Resource {
				expires := time.Now().UTC().Add(time.Hour)
				token, err := types.NewProvisionToken("token",
		...
		},
		{
			...
			crud: func(context.Context) types.Resource {
				...
				require.Nil(t, s.Users().DeleteUser(ctx, user.GetName()))
				return out
			},
		},
		{
		...
		},
		{
			...
			crud: func(context.Context) types.Resource {
				...
				rc.SetConnectionStatus(teleport.RemoteClusterStatusOffline)
				require.NoError(t, err)
				require.Nil(t, s.PresenceS.CreateRemoteCluster(rc))
...
func (s *ServicesTestSuite) NetworkRestrictions(t *testing.T, opts ...Option) {
	...
	fixtures.DeepCompare(t, expected.GetAllow(), actual.GetAllow())
	fixtures.DeepCompare(t, expected.GetDeny(), actual.GetDeny())
...
waitLoop:
	for {
		...
		case event := <-w.Events():
			...
			fixtures.DeepCompare(t, resource, event.Resource)
			break waitLoop
		}
	}
}
