...
resource "aws_s3_object" "grafana_teleport_dashboard" {
  ...
  etag   = filemd5("${path.module}/assets/health-dashboard.json")
}
...
resource "aws_s3_object" "grafana_teleport_nginx" {
  ...
  etag   = filemd5("${path.module}/assets/grafana-nginx.conf")
}
...
resource "aws_s3_object" "grafana_teleport_nginx_acm" {
  ...
  etag   = filemd5("${path.module}/assets/grafana-nginx-acm.conf")
}
