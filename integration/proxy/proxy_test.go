...
func TestKubeIPPinning(t *testing.T) {
	...
	const (
		//rootKubeCluster = "kube-root.teleport.cluster.local"
		//leafKubeCluster = "kube-leaf.teleport.cluster.local"
		rootKubeCluster = "kube.teleport.cluster.local"
		leafKubeCluster = "kube.teleport.cluster.local"
		k8User          = "alice@example.com"
		k8RoleName      = "kubemaster"
	)
	...
	kubeConfigPathRoot := mustCreateKubeConfigFile(t, k8ClientConfig(kubeAPIMockSvrRoot.URL, rootKubeCluster))
	kubeConfigPathLeaf := mustCreateKubeConfigFile(t, k8ClientConfig(kubeAPIMockSvrLeaf.URL, leafKubeCluster))
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			k8Client, _, err := kube.ProxyClient(kube.ProxyConfig{
				...
				KubeUsers:           kubeRoleSpec.Allow.KubeGroups,
				KubeGroups:          kubeRoleSpec.Allow.KubeUsers,
				CustomTLSServerName: tc.serverName,
				TargetAddress:       suite.root.Config.Proxy.WebAddr,
