...
func main() {
	...
	aptly, err := SetupAptly("./apt")
	if err != nil {
		log.WithError(err).Fatal("Failed initing aptly")
	}
	defer aptly.Close()

	//downloadExistingRepo(awsSession, *args.bucketName, *args.localBucketPath)

	repos, err := aptly.CreateReposFromBucket(*args.localBucketPath)
	if err != nil {
		log.WithError(err).Fatal("Failed loading repositories into aptly")
	}

	// make sure existing repos know about their own existing artifacts
	for _, repo := range repos {
		log.Debugf("Adding existing artifacts to %s", repo.Name())
		debPath := filepath.Join(*args.localBucketPath, repo.PublishedRepoRelativePath())
		err = aptly.ImportDeb(repo, debPath)
		if err != nil {
			log.WithError(err).Fatalf("Failed adding existing artifacts to repo")
		}
	}
